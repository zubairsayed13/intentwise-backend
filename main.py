...
from secrets_loader import load_secrets, SLACK_BOT_TOKEN, SLACK_WEBHOOK_URL, SLACK_CHANNEL_ID
...
_RESP_CACHE = {}  # key → (data, timestamp)
_RESP_CACHE_TTL = 60  # seconds

def cached_response(key: str, ttl: int = _RESP_CACHE_TTL):
    """Decorator-style cache for expensive endpoint results."""
    def get():
        ...
        return None
    def set_(data):
        _RESP_CACHE[key] = (data, _time.time())
    return get, set_
        ...
        if alerts:
            ...
            try:
                await slack_send_rich(
                    text="\n".join(lines),
                    nt="anomaly_alert",
                    entity_id=f"anomaly_{datetime.date.today().isoformat()}",
                )
            except Exception:
                pass
...
async def lifespan(app):
    import asyncio as _asyncio
    # Start Slack Socket Mode (async — Windows safe)
    try:
        from slack_socket import start_socket_mode_async
        await start_socket_mode_async()
    except Exception as e:
        logger.warning(f"[lifespan] Slack Socket Mode not started: {e}")
...
# ── Slack proxy — frontend routes all notifications here to avoid CORS ────────
@app.post("/api/notify/slack")
async def notify_slack(payload: dict):
    """
    Proxy endpoint: browser calls this instead of hitting Slack directly (CORS fix).
    Priority: SLACK_BOT_TOKEN (chat.postMessage) → SLACK_WEBHOOK_URL (env) → webhook_url (from payload).
    All three come from secrets_loader / .env — no credentials ever sent from browser.
    """
    text        = payload.get("text", "")
    channel     = payload.get("channel") or SLACK_CHANNEL_ID or ""
    blocks      = payload.get("blocks")
    attachments = payload.get("attachments")
    # webhook_url in payload is a last-resort fallback the frontend may send
    fallback_url = payload.get("webhook_url", "")

    try:
        async with httpx.AsyncClient(timeout=10) as client:

            # ── Bot token path (preferred) ──────────────────────────────
            if SLACK_BOT_TOKEN:
                if not channel:
                    return {"ok": False, "error": "SLACK_CHANNEL_ID not set and no channel provided"}
                ...
                return {"ok": False, "error": data.get("error", "slack api error")}

            # ── Webhook path (env or frontend fallback) ─────────────────
            url = SLACK_WEBHOOK_URL or fallback_url
            if not url:
...
@app.get("/api/notify/slack/status")
async def slack_status():
    """Let the frontend know which credentials are active — never exposes values."""
    method = "bot_token" if SLACK_BOT_TOKEN else ("webhook" if SLACK_WEBHOOK_URL else "none")
    return {
        "configured":      method != "none",
        "method":          method,
        "has_token":       bool(SLACK_BOT_TOKEN),
        "has_webhook":     bool(SLACK_WEBHOOK_URL),
        "has_channel":     bool(SLACK_CHANNEL_ID),
        "default_channel": SLACK_CHANNEL_ID or None,
    }


# ── Centralised Slack rich-message helper ─────────────────────────────────────
# All backend Slack sends go through here.  Builds Block Kit messages with
# interactive buttons so every notification type is actionable from Slack.
#
# Notification types  (nt=)        Buttons surfaced
# ─────────────────────────────────────────────────
# anomaly_alert                    acknowledge · view_observatory · snooze_1h
# sop_gate_pending                 gate_approve · gate_reject
# sop_run_complete                 view_report · run_again
# remediation_complete             verify_fix · rollback
# workflow_failure                 wf_acknowledge · wf_run_again · snooze_1h
# workflow_agent_step              view_details · skip_continue
# demo_validation_alert            acknowledge · trigger_recheck
# pipeline_digest                  view_dashboard · configure_alerts
# circuit_breaker                  reenable_pipeline · view_run_history
# (plain)                          — (text-only fallback) ─────────────────────

_snooze_store: dict = {}   # { entity_id: snooze_until_iso }

_WZ_BUTTON_MAP: dict = {
    "anomaly_alert": [
        ("Acknowledge",         "acknowledge",      "primary"),
        ("View Observatory",    "view_observatory", None),
        ("Snooze 1 h",          "snooze_1h",        None),
    ],
    "sop_gate_pending": [
        ("Approve ✓",           "gate_approve",     "primary"),
        ("Reject ✗",            "gate_reject",      "danger"),
    ],
    "sop_run_complete": [
        ("View Full Report",    "view_report",      "primary"),
        ("Run Again",           "run_again",        None),
    ],
    "remediation_complete": [
        ("Verify Fix",          "verify_fix",       "primary"),
        ("Rollback",            "rollback",         "danger"),
    ],
    "workflow_failure": [
        ("Acknowledge",         "wf_acknowledge",   "primary"),
        ("Run Again",           "wf_run_again",     None),
        ("Snooze 1 h",          "snooze_1h",        None),
    ],
    "workflow_agent_step": [
        ("View Details",        "view_details",     "primary"),
        ("Skip & Continue",     "skip_continue",    None),
    ],
    "demo_validation_alert": [
        ("Acknowledge",         "acknowledge",      "primary"),
        ("Trigger Re-check",    "trigger_recheck",  None),
    ],
    "pipeline_digest": [
        ("View Full Dashboard", "view_dashboard",   "primary"),
        ("Configure Alerts",    "configure_alerts", None),
    ],
    "circuit_breaker": [
        ("Re-enable Pipeline",  "reenable_pipeline","primary"),
        ("View Run History",    "view_run_history", None),
    ],
}


async def slack_send_rich(
    *,
    text:       str,
    nt:         str  = "plain",     # notification type — see table above
    entity_id:  str  = "",          # run_id / wf_id / gate token etc.
    channel:    str  = "",
    extra_blocks: list = None,       # prepend extra blocks before buttons
) -> dict:
    """
    Central Slack send.  Builds a Block Kit message with contextual buttons
    then dispatches via bot token (preferred) or webhook fallback.
    Returns {ok, ts?, error?}.
    """
    import httpx as _httpx

    ch  = channel or SLACK_CHANNEL_ID or ""
        ...
        async with _httpx.AsyncClient(timeout=8) as cl:
            if SLACK_BOT_TOKEN:
                ...
                r = await cl.post(
                    ...
                    headers={"Authorization": f"Bearer {SLACK_BOT_TOKEN}",
                             "Content-Type": "application/json"},
                    json={"channel": ch, "text": text, "blocks": blocks},
                )
                d = r.json()
                return {"ok": d.get("ok", False), "ts": d.get("ts"), "error": d.get("error")}
            if SLACK_WEBHOOK_URL:
                r = await cl.post(SLACK_WEBHOOK_URL,
                                  json={"text": text, "blocks": blocks},
                                  headers={"Content-Type": "application/json"})
                return {"ok": r.status_code == 200, "error": None if r.status_code == 200 else r.text}
    except Exception as exc:
        logger.warning(f"[slack_send_rich] {exc}")
        return {"ok": False, "error": str(exc)}
    return {"ok": False, "error": "no credentials"}


def _slack_fire(*, text, nt="plain", entity_id="", channel="", extra_blocks=None):
    """Sync wrapper around slack_send_rich — safe to call from non-async LangGraph nodes."""
    import asyncio as _aio
    coro = slack_send_rich(text=text, nt=nt, entity_id=entity_id,
                           channel=channel, extra_blocks=extra_blocks)
    try:
        loop = _aio.get_event_loop()
        if loop.is_running():
            # Already inside an event loop (uvicorn) — schedule as fire-and-forget
...
_AI_CHAT_CACHE: dict = {}  # hash → (response, timestamp)
_AI_CHAT_CACHE_TTL = 300  # 5 min — same prompt reuses answer

def _select_model(req) -> str:
    if len(req.messages) > 1: return "gpt-4o"
    if req.max_tokens > 500: return "gpt-4o"
    if req.temperature > 0.5: return "gpt-4o"
    return "gpt-4o-mini"
...
async def ai_chat(req: ChatRequest):
    ...
    cached = _AI_CHAT_CACHE.get(cache_key)
    if cached and _time.time() - cached[1] < _AI_CHAT_CACHE_TTL:
        return cached[0]
    model = _select_model(req)
    async with httpx.AsyncClient(timeout=60) as client:
        payload = {"model": model, "max_tokens": req.max_tokens, "temperature": req.temperature, "messages": messages}
        r = await client.post(
        ...
        result = {"content": [{"type": "text", "text": text}], "model": model}
        if len(req.messages) == 1:
            _AI_CHAT_CACHE[cache_key] = (result, _time.time())
        return result
...
async def remediation_notify(payload: dict):
    ...
    rows     = payload.get("rows_affected", 0)
    before   = payload.get("before", 0)
    after    = payload.get("after", 0)
    run_id   = payload.get("run_id", "rem")
    text     = (
        f"*🔧 Remediation Complete — mws.orders*\n"
        f"{payload.get('message','')}\n"
        f"Fix: `{fix_type}` · Rows: *{rows}* · {before}→{after}"
    )
    result = await slack_send_rich(text=text, nt="remediation_complete", entity_id=run_id)
    return {"sent": result.get("ok"), "error": result.get("error")}
...
{{"strategy":"auto_fix|needs_approval|escalate","fix_actions":["redrive","recopy"],"needs_approval":true|false,"escalate_reason":"","summary":"one sentence","risk_reason":"why approval needed or empty"}}"""
    ...
    def node_request_approval(state: ReportTriageState) -> dict:
        ...
        try:
            _slack_fire(text=msg, nt="workflow_agent_step", entity_id=token)
    ...
    def node_notify(state: ReportTriageState) -> dict:
        ...
        notified = False
        try:
            _slack_fire(text=msg, nt="remediation_complete",
                        entity_id=state.get("run_id","rem"))
            notified = True
        except Exception: pass
...
{{"strategy":"auto_fix|escalate|mixed","auto_fix_ids":["WA-NULL-x"],"escalate_ids":["WA-DUPE-y"],"escalate_reason":"","summary":"one sentence"}}"""
    ...
    def table_node_notify(state: TableAgentState) -> dict:
        ...
        try:
            _slack_fire(text=msg, nt="remediation_complete",
                        entity_id=f"{state.get('schema','')}.{state.get('table','')}")
            notified = True
        except Exception: pass
    ...
    for step_result in results:
        ...
        for branch in branches:
            ...
            if action == "stop":
                ...
                try:
                    msg_b = (f"⚠️ *{wf.get('name','')}* — `{step_result['table']}` "
                             f"({agent_name}) · {len(step_issues)} issue(s) found.")
                    await slack_send_rich(
                        text=msg_b, nt="workflow_agent_step",
                        entity_id=f"{wf.get('id','wf')}_{step_result['table']}",
                    )
    ...
    if total_issues > 0:
        try:
            ...
            msg = (f"⚠️ *{wf_name}* workflow completed\n"
                   f"{total_issues} issue(s) across {len(tables)} table(s)\n"
                   f"Schema group: {sg} · DB: {db_key}")
            r = await slack_send_rich(
                text=msg, nt="workflow_failure", entity_id=wf.get("id", run_id),
            )
            notified = r.get("ok", False)
        except Exception: pass
...
async def _sop_finalize(run_state: dict) -> dict:
    ...
    try:
        r = await slack_send_rich(
            text=f"✅ *{summary}*", nt="sop_run_complete", entity_id="sop_today",
        )
        notified = r.get("ok", False)
    except Exception: pass
    ...
    async def _notify_gate_pending(gate_num: int):
        ...
        try:
            ...
            msg = (
                ...
                f"Open the dashboard or use the buttons below."
            )
            await slack_send_rich(
                text=msg, nt="sop_gate_pending", entity_id=token,
            )
        except Exception:
            pass
...
async def run_demo_check():
    ...
    if overall_status in ("fail", "warn"):
        ...
        try:
            await slack_send_rich(
                text="\n".join(msg_lines), nt="demo_validation_alert",
                entity_id=f"demo_{datetime.date.today().isoformat()}",
            )
        except Exception:
            pass
...
async def _run_workflow_checks(wf: dict, triggered_by: str = "manual") -> dict:
    ...
    for chk in checks:
        ...
        try:
            if _ADDONS_LOADED:
                rows      = execute_on_source(source_id, sql)
                # Normalize: ensure rows are dicts not lists
                if rows and not isinstance(rows[0], dict):
                    rows = [dict(enumerate(r)) if not hasattr(r, 'keys') else dict(r) for r in rows]
                row_dicts = rows
...
async def send_digest(payload: dict = {}):
    ...
    try:
        ...
        resp = await slack_send_rich(text=msg, nt="pipeline_digest", entity_id="digest")

        # Store digest record
        digest = {
            "sent_at": datetime.datetime.utcnow().isoformat(),
            "days": days, "hit_rate": hit_rate,
            "slack_ok": resp.get("ok", False),
        }
        try:
...
async def share_result_to_slack(payload: dict = {}):
    ...
    try:
        r = await slack_send_rich(text=msg, nt="workflow_failure", entity_id=run_id)
        return {"sent": r.get("ok"), "error": r.get("error")}
    except Exception as e:
        return {"error": str(e)}
...
async def slack_send(request: Request):
    """Send a message to Slack via webhook. Used by frontend alert system."""
    try:
        body    = await request.json()
        webhook = body.get("webhook") or SLACK_WEBHOOK_URL or os.getenv("SLACK_WEBHOOK_URL", "")
        if not webhook:
            return {"ok": False, "error": "No webhook URL configured"}
        payload = {k: v for k, v in body.items() if k != "webhook"}
        async with httpx.AsyncClient(timeout=10) as client:
...
async def slack_actions(request: Request):
    """
    Handle Slack interactivity button callbacks.
    action_id namespace: wz_{action}_{entity_id}
    """
    try:
        ...
        raw_action = actions[0].get("action_id", "")   # wz_acknowledge_abc123
        entity_id  = actions[0].get("value", "")
        user       = payload.get("user", {}).get("name", "unknown")
        # Strip wz_ prefix and entity suffix to get clean action
        parts  = raw_action.split("_", 2)          # ["wz", "action", "entity"]
        action = parts[1] if len(parts) >= 2 else raw_action

        logger.info(f"[slack_actions] action={action} entity={entity_id} user={user}")
        result: dict = {"ok": True, "action": action, "entity_id": entity_id}

        # ── Gate approve / reject ─────────────────────────────────────────────
        if action in ("gate_approve", "gate_reject"):
            decision = "approved" if action == "gate_approve" else "rejected"
            entry = _sop_gate_store.get(entity_id)
            if entry and entry.get("decision") is None:
                entry["decision"] = decision
                entry["event"].set()
                run_id   = entry.get("run_id", "")
                gate_num = entry.get("gate_num", 0)
                ...
                logger.info(f"[slack_actions] gate {entity_id} → {decision} by {user}")
            else:
                result["warning"] = "gate not found or already resolved"

        # ── Acknowledge — log to DB ───────────────────────────────────────────
        elif action == "acknowledge":
            try:
                conn = get_connection()
                _ensure_uploads_schema(conn)
                conn.execute("""
                    CREATE TABLE IF NOT EXISTS wz_uploads._slack_acks
                    (id SERIAL PRIMARY KEY, entity_id TEXT, action TEXT,
                     slack_user TEXT, acked_at TIMESTAMPTZ DEFAULT NOW())
                """)
                conn.execute(
                    "INSERT INTO wz_uploads._slack_acks (entity_id, action, slack_user) VALUES (%s,%s,%s)",
                    (entity_id, action, user),
                )
                conn.commit(); conn.close()
            except Exception as db_err:
                logger.warning(f"[slack_actions] ack db error: {db_err}")

        # ── Snooze 1 hour ─────────────────────────────────────────────────────
        elif action == "snooze_1h":
            until = (datetime.datetime.utcnow() + datetime.timedelta(hours=1)).isoformat()
            _snooze_store[entity_id] = until
            result["snoozed_until"] = until
            logger.info(f"[slack_actions] snoozed {entity_id} until {until}")

        # ── Re-enable pipeline (circuit breaker) ─────────────────────────────
        elif action == "reenable_pipeline":
            # entity_id is the pipeline id stored in localStorage on the frontend.
            # We record intent here; the frontend polls and clears the breaker.
            _snooze_store.pop(entity_id, None)   # clear any snooze
            result["reenable_requested"] = True
            result["note"] = "Frontend will clear circuit breaker on next poll"
            logger.info(f"[slack_actions] reenable pipeline {entity_id} by {user}")

        # ── Workflow run again ────────────────────────────────────────────────
        elif action == "wf_run_again":
            wf = _CUSTOM_WORKFLOWS.get(entity_id)
            if wf:
                asyncio.create_task(_run_custom_workflow(wf))
                result["rerun_queued"] = True
            else:
                result["warning"] = f"workflow {entity_id} not found"

        # ── SOP run again ─────────────────────────────────────────────────────
        elif action == "run_again":
            if entity_id.startswith("sop"):
                new_id = f"sop_{datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
                _sop_runs[new_id] = {
                    "run_id": new_id, "status": "running",
                    "started_at": datetime.datetime.utcnow().isoformat(),
                    "trace": [], "force_full": True,
                }
                asyncio.create_task(_run_sop(new_id))
                result["new_run_id"] = new_id

        # ── Trigger re-check (demo validation) ───────────────────────────────
        elif action == "trigger_recheck":
            asyncio.create_task(_run_demo_validation_internal())
            result["recheck_queued"] = True

        # ── All other actions — log only ──────────────────────────────────────
        else:
            logger.info(f"[slack_actions] unhandled action={action} entity={entity_id}")

        return result

    except Exception as e:
        logger.error(f"[slack_actions] error: {e}")
        return {"ok": False, "error": str(e)}


# ── Expose snooze state so frontend can check ─────────────────────────────────
@app.get("/api/slack/snooze/{entity_id}")
async def get_snooze(entity_id: str):
    until = _snooze_store.get(entity_id)
    if not until:
        return {"snoozed": False}
    if datetime.datetime.utcnow().isoformat() > until:
        _snooze_store.pop(entity_id, None)
        return {"snoozed": False}
    return {"snoozed": True, "until": until}


@app.post("/api/email/test")
async def email_test(request: Request):
    """Test SMTP email configuration. Returns ok:true if successful."""
    try:
        import smtplib
        from email.mime.text import MIMEText
        body = await request.json()
        msg  = MIMEText(body.get("body", "WiziAgent test email"))
        msg["Subject"] = body.get("subject", "WiziAgent test")
        msg["From"]    = body.get("senderEmail", "")
        msg["To"]      = body.get("recipients", "").split(",")[0].strip()
        with smtplib.SMTP(body.get("smtpHost",""), int(body.get("smtpPort", 587))) as s:
            s.starttls()
            s.login(body.get("senderEmail",""), body.get("password",""))
            s.send_message(msg)
        return {"ok": True}
    except Exception as e:
        return {"ok": False, "error": str(e)}
