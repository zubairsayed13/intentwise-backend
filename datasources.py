...
_QUERY_CACHE: dict = {}  # (key, sql) → (rows, timestamp)
QUERY_CACHE_TTL = 120    # seconds — tune per environment

def _cache_get(key: str, sql: str):
    entry = _QUERY_CACHE.get((key, sql))
    if entry and time.time() - entry[1] < QUERY_CACHE_TTL:
        return entry[0]
    return None

def _cache_set(key: str, sql: str, rows: list):
    _QUERY_CACHE[(key, sql)] = (rows, time.time())

# ── Connection pool (reuse connections per key) ───────────────────────────────
_CONN_POOL: dict = {}  # key → connection

def _get_pooled_conn(key: str):
...
def execute_on_source(key: str, sql: str, params=None, **kwargs) -> list:
    ...
    # Check cache first (only for param-free SELECT queries)
    if not params:
        cached = _cache_get(key, sql)
    ...
    if db_type == "bigquery":
        rows = _execute_bigquery(key, sql)
        if not params: _cache_set(key, sql, rows)
        return rows

    try:
        conn = _get_pooled_conn(key)
        if db_type == "mysql":
